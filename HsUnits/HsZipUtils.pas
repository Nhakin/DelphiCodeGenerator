...
Type
  ...
  IHsMemoryZipper = Interface(IInterfaceEx)
    ...
    Procedure AddFromStream(Const AFileName : String; AFromStream : TStream);
    ...
    Procedure ExtractFiles(Const AFileMask, ATargetDir : String);
  ...
  Protected
    ...
    Function  Add() : IHsMemoryZipper; OverLoad;
...
Type
  TZipperProgress = Class(TForm, IZipProgress)
  Private
  ...
  Protected
    Property IntfImpl : TInterfaceExImplementor Read GetImpl Implements IZipProgress;
...
Constructor TZipperProgress.Create(AOwner : TComponent);
Var lPanel : TPanel;
Begin
  CreateNew(AOwner);
...
Function TZipperProgress.GetImpl() : TInterfaceExImplementor;
Begin
  If Not Assigned(FIntfImpl) Then
    FIntfImpl := TInterfaceExImplementor.Create(Self, True);
  Result := FIntfImpl;
End;

Procedure TZipperProgress.DoClose(Var Action : TCloseAction);
Begin
  Action := caFree;
End;

Function TZipperProgress.GetItemProgress() : Byte;
Begin
  Result := FItemProgress.Position;
End;

Procedure TZipperProgress.SetItemProgress(Const AItemProgress : Byte);
Begin
  FItemProgress.Position := AItemProgress;
  Application.ProcessMessages();
End;

Function TZipperProgress.GetArchiveProgress() : Byte;
Begin
  Result := FArchiveProgress.Position;
End;

Procedure TZipperProgress.SetArchiveProgress(Const AArchiveProgress : Byte);
Begin
  FArchiveProgress.Position := AArchiveProgress;
  Application.ProcessMessages();
End;

Function TZipperProgress.GetCurOperation() : String;
Begin
  Result := FlblStatus.Caption;
End;

Procedure TZipperProgress.SetCurOperation(Const ACurOperation : String);
Begin
...
Begin
  ...
  Begin
    FZipProgress := TZipperProgress.Create(Nil);
    FZipProgress.Show();
  End;
