...
Type
  ...
  Strict Private
    ...
    Procedure SetCheckSupport(Const ACheckSupport : Boolean);
...
Begin
  ...
  Begin
    ...
    Begin
      ...
      If Header.Columns[0].CheckBox Then
        CellRect.Left := CellRect.Left + CheckImages.Width + TextMargin;
...
Var lNode : PVirtualNode;
    ...
    lNbChecked : Integer;
Begin
  FDropDown.PreventCloseUp := False;
...
Function TCustomVirtualStringTreeDropDown.GetCheckSupport() : Boolean;
Begin
  Result := toCheckSupport In FVST.TreeOptions.MiscOptions;
End;

Procedure TCustomVirtualStringTreeDropDown.SetCheckSupport(Const ACheckSupport : Boolean);
Var lNode : PVirtualNode;
Begin
  If FVST.Header.Columns[0].CheckBox <> ACheckSupport Then
  Begin
    FVST.Header.Columns[0].CheckBox := ACheckSupport;

    If ACheckSupport Then
      FVST.TreeOptions.MiscOptions := FVST.TreeOptions.MiscOptions + [toCheckSupport]
    Else
      FVST.TreeOptions.MiscOptions := FVST.TreeOptions.MiscOptions - [toCheckSupport];
...
Begin
  ...
  While Assigned(lNode) Do
  Begin
    If FVST.Header.Columns[0].CheckBox Then
      lNode.CheckType := ctCheckBox
    Else
      lNode.CheckType := ctNone;
